...
data Status
  ...
  | LoadedWords [String]
  | Res Result
  deriving (Show) -- per test interattivo
...
--    - chiave estratta
result ::
  [String] -> -- lista delle parole mancanti nella matrice
...
readWord dir (s : xs) schema ((r, c) : xm) =
  let nm = nextPos dir (r, c) : (r, c) : xm
   in if canMove (r, c) dir (length xs) schema
        then case schema `at` (r, c) of
          Nothing -> [] -- Just [(2,2)] -- Nothing --
          Just v ->
            if s /= v
              then [] -- Just ([(3,3), (100,100)] ++ ((r,c):xm)) -- Nothing --
